...
public class DetailEventRenderer extends RendererBase implements TimeBarRenderer, TimeBarRenderer2 {
    ...
    private void defaultDraw(GC gc, Rectangle drawingArea, TimeBarViewerDelegate delegate, Interval interval,
        ...
        if(InteractionUtil.isBrowser(event.getApplication()))
        {
        	color = SWT.COLOR_BLUE;
        }
        else if("eclipse.exe".equals(event.getApplication()) || "javaw.exe".equals(event.getApplication()) || 
        		"devenv.exe".equals(event.getApplication()))
        {
        	color = SWT.COLOR_CYAN;
        }
        else if (event.getColor() != null && event.getColor().equals("red")) {
        	color = SWT.COLOR_RED;
        ...
        //Image im = new Image(gc.getDevice(), DetailEventRenderer.class.getResourceAsStream("/icons/firefox.png"));
        //gc.drawImage(im, da.x, da.y);
